...
def lambda_handler(event, context):
    ...
    for kb_id in knowledge_base_ids:
        try:
            ...
            while retry_count < max_retries:
                try:
                    ...
                    if retry_count >= max_retries:
                        raise
                    logger.info(
                        f"Knowledge base {kb_id} not found yet, retrying in 5 seconds (attempt {retry_count}/{max_retries})"
                    )
                    time.sleep(5)
            ...
            while retry_count < max_retries:
                try:
                    ...
                    if retry_count >= max_retries:
                        raise
                    logger.info(
                        f"Data sources for knowledge base {kb_id} not found yet, retrying in 5 seconds (attempt {retry_count}/{max_retries})"
                    )
                    time.sleep(5)

            data_source_ids = [
                ds["dataSourceId"] for ds in data_sources.get("dataSourceSummaries", [])
            ]

            logger.info(
                f"Knowledge Base: {kb_name} ({kb_id}) has {len(data_source_ids)} data sources"
            )

            # If no data sources found, log a warning
            if not data_source_ids:
                logger.warning(
                    f"No data sources found for knowledge base {kb_name} ({kb_id}). Knowledge base may not be completely set up yet."
                )
            ...
            for ds_id in data_source_ids:
                ...
                for job in ingestion_jobs.get("ingestionJobSummaries", []):
                    if (
                        ...
                        and job.get("completedAt", datetime.min) > one_day_ago
                    ):
                        recent_success = True
                        break
                ...
                if not recent_success:
                    logger.info(
                        f"No recent successful ingestion job for data source {ds_name} ({ds_id}). Starting new job."
                    )
                    ...
                    while retry_count < max_retries and not success:
                        try:
                            ...
                            if error_code in [
                                ...
                                if retry_count < max_retries:
                                    logger.warning(
                                        f"Rate limited when starting ingestion job. Retrying in {backoff_time}s (attempt {retry_count}/{max_retries})"
                                    )
                                    time.sleep(backoff_time)
                                    backoff_time *= 2  # Exponential backoff
                                else:
                                    logger.error(
                                        f"Failed to start ingestion job after {max_retries} attempts: {str(e)}"
                                    )
                                    kb_results.append(
                                        {
                                            "dataSourceId": ds_id,
                    ...
                    logger.info(
                        f"Recent successful ingestion job found for data source {ds_name} ({ds_id})"
                    )
                    kb_results.append(
                        {
                            "dataSourceId": ds_id,
            ...
            results[kb_id] = {"knowledgeBaseName": kb_name, "dataSources": kb_results}

        except Exception as e:
            logger.error(
                f"Error processing knowledge base {kb_id}: {str(e)}", exc_info=True
            )
            results[kb_id] = {
                "error": str(e),
                "knowledgeBaseId": kb_id,
