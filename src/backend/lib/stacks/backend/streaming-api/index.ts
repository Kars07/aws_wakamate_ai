...
export class StreamingApi extends Construct {
  ...
  constructor(scope: Construct, id: string, props: StreamingApiProps) {
      ...
      [
        {
          ...
          reason:
            "AmplifyGraphqlApi requires the AWSAppSyncPushToCloudWatchLogs policy for logging.",
        },
        {
          id: "AwsSolutions-S1",
          reason:
            "AmplifyGraphqlApi-created buckets do not require server access logs.",
        },
        {
          id: "AwsSolutions-S10",
          reason:
            "AmplifyGraphqlApi-created buckets do not require requests to use SSL.",
        },
      ],
      true
    );
    ...
    Object.values(amplifiedGraphApi.resources.cfnResources.cfnTables).forEach(
      (table) => {
    ...
    const resolverFunction = new CommonNodejsFunction(
      this,
      "resolverFunction",
      {
