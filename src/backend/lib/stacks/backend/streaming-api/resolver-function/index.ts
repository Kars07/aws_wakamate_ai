  ...
  while (true) {
    try {
      ...
      const isRetryableError =
        ...
        ((typeof error === "object" &&
          "name" in error &&
          errorTypesToRetry.includes(error.name)) ||
          (typeof error === "object" &&
            "errorType" in error &&
            errorTypesToRetry.includes(error.errorType)) ||
          (error.constructor &&
            errorTypesToRetry.includes(error.constructor.name)));

      if (isRetryableError && retries < maxRetries) {
        const errorName =
          error.name || error.errorType || error.constructor?.name || "Unknown";
        console.log(
          `${errorName} detected, retry attempt ${
            retries + 1
